...
if st.session_state.current_plan and st.session_state.current_constraints:
    ...
    # 对齐新版 Pydantic 模型中的安全和天气风控机制
    if plan.safety_review.requires_human_confirmation:
        st.error(f"⚠️ **Human-in-the-loop 安全风控提示**: {plan.safety_review.warning_message}")
    else:
        # 展示天气与备选方案说明
        if plan.alternatives_for_weather:
            st.info(f"🌦️ **自适应天气备选预案**: {plan.alternatives_for_weather}")
